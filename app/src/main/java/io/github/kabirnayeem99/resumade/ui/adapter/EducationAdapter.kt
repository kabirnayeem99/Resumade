...
class EducationAdapter(
    val onSaveButtonClick: (Education) -> Unit,
    val onDeleteButtonClick: (Education) -> Unit,
    val onEditButtonClick: (Education) -> Unit
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        position: Int
    ): EducationAdapter.EducationViewHolder {
        return EducationViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.card_education, parent, false)
        )
    }
    ...
    inner class EducationViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val instituteNameWrapper: TextInputLayout =
            itemView.findViewById(R.id.educationInstituteNameWrapper)
        private val instituteName: TextInputEditText =
            itemView.findViewById(R.id.educationInstituteName)
        private val degreeWrapper: TextInputLayout =
            itemView.findViewById(R.id.educationDegreeWrapper)
        private val degree: TextInputEditText = itemView.findViewById(R.id.educationDegree)
        private val performanceWrapper: TextInputLayout =
            itemView.findViewById(R.id.educationPerformanceWrapper)
        private val performance: TextInputEditText =
            itemView.findViewById(R.id.educationPerformance)
            ...
            saveButton.apply {
                setOnClickListener {
                    if (mEducation.saved) {
                        ...
                        // Save Mode
                        val tempInstituteName =
                            this@EducationViewHolder.instituteName.text?.toString()
                                ?: ""
                        val tempDegree = this@EducationViewHolder.degree.text?.toString() ?: ""
                        val tempPerformance =
                            this@EducationViewHolder.performance.text?.toString() ?: ""
                        val tempYear =
                            this@EducationViewHolder.yearOfGraduation.text?.toString() ?: ""
                        ...
                        when {
                            tempYear.trim().toLongOrNull() == null -> {
                                yearWrapper.error = "Year must contain numbers only"
                                passed = false
                            }
                            tempYear.trim().isEmpty() -> {
